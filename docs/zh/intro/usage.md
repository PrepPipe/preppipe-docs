...
为方便在只有剧本时生成可玩的原型，语涵编译器内嵌了多组背景与立绘素材。这些素材免费可商用，保存后也可独立于本程序使用。具体素材列表可在程序的导航页找到。您可以在程序的导航页选中相应素材进入详情页，在这里可以查看不同差分、进行自定义修改（仅部分模板支持），并且在预览区右键的下拉菜单可以保存当前图片。

如果您觉得有其他素材会很常用，欢迎向我们反馈，我们会酌情约稿、补充素材。

注：本项目的内嵌素材是成组准备的，所有差分（背景的白天、傍晚、夜晚等）都是事先准备的、并不是程序生成的。如果您使用自己的图片且希望有差分，请自行准备这些差分。您可以使用 AI 工具或是图片处理程序等。
...
目前语涵编译器还内嵌了两组临时的角色立绘:

* DT1 （短发女生）
* DT2 （长发女生）

这些立绘还没确定正式名称，目前想在剧本中引用的话只能用代号。使用方式如下：

!!! example "【声明角色】与预制角色立绘"

    【声明角色：甲】

      * 立绘：预设（DT1）

程序支持给立绘的部分区域（头发、衣服等）改色，但暂时不支持在剧本中修改。如果需要修改后的版本，请在素材的详情页自定义修改后保存图片。
...
除了上面的三组背景模板外，本项目还准备了部分常见场景，包括学校、教室、卧室、客厅等。使用方法与背景模板相同，唯一区别是“预设”不支持“屏幕”等参数，括号内只包含预制背景的名字。所有预制背景都可在程序内的导航页进行预览。
