...
!!! note "单向滚动"
    ...
    FairyGUI中的列表可以选择只允许在水平或垂直方向滚动，但Ren'Py中的viewport或vpgrid无法实现相同效果。因此将列表的溢出处理设置为“垂直滚动”、“水平滚动”、“自由滚动”后，转换器生成的Ren'Py项目中都可以在两个方向上自由滚动。

点击溢出处理右侧的齿轮按钮会弹出二级选项。“滚动条组件”下可以指定垂直滚动条和水平滚动条，**其他设置项都不会在最终Ren'Py项目生效**。
