...
!!! tip "FairyGUI组件最小尺寸"
    ...
    FariGUI编辑器新建滑动条的最小宽度或最小高度为10像素。如果引用的图片资源宽度或高度小于10像素，创建的滑动条对应的宽度或高度固定为10像素，需要手动修改组件尺寸并调整组件内元素的位置。
...
缩放和剪裁两种方式在实际的不同游戏项目中都可能用到。若需要滑动条前景使用剪裁，请在生成的bar样式中手动将*bar_resizing*的值改为False。
