...
“剪裁图片边缘空白”会在发布纹理集时，重新计算png图片非透明像素的最小包围框并裁剪。该功能可以节约一些纹理集的存储空间，但可能会导致使用“UI资源转换功能”后Ren'Py显示效果有异常。

## 额外步骤

FairyGUI发布资源时，无论字体是否设置为导出，都**不包含字体文件**。需要手动将字体文件复制到Ren'Py项目的game目录或game/fonts目录中。
